...
f);console.error("[LWF] load error: %o",a.error);console.error("[LWF] loaded count: %d/%d",a.loadedCount,a.total)};h.prototype.onLoad=function(a){var i=this,e=i.privateData._loaderData,f=e.loader,c=e.stageEventReceiver;i.privateData=null;if(a){if(_.isFunction(e.callback.onLoad))e.callback.onLoad(a);var g,d,t,h,D,l=this.stage,n=0,s=0,u=1,v=0,w=0,E=b.performance.now(),F=0,x=E,G=0;a.rootMovie.moveTo(f.rootOffset.x,f.rootOffset.y);g=function(){try{if(a){var d=b.devicePixelRatio;"useWebkitCSSRenderer"===
f.getRenderer()&&(d=1);var q=a.width,r=a.height,h=b.innerWidth,t=b.innerHeight;p&&(b.innerWidth>b.screen.width&&(h=b.screen.width),b.innerHeight>b.screen.height&&(t=b.screen.height));if(q>h||e.resizeStretch)q=h;if(r>t||e.resizeStretch)r=t;if(n!==q||s!==r)if(i.fitForWidth?(i.widthLimit&&(q=q>i.widthLimit?i.widthLimit:q),v=Math.round(q),w=Math.round(q*a.height/a.width)):(i.heightLimit&&(r=r>i.heightLimit?i.heightLimit:r),v=Math.round(r*a.width/a.height),w=Math.round(r)),c.style.width=l.style.width=
v+"px",c.style.height=l.style.height=w+"px",c.width=l.width=Math.floor(v*d),c.height=l.height=Math.floor(w*d),i.fitForWidth?(u=v/l.width,a.property.clear(),a.fitForWidth(l.width,l.height)):(u=w/l.height,a.property.clear(),a.fitForHeight(l.width,l.height)),n=q,s=r,f.displayDivId){var k=document.getElementById(f.displayDivId);k.style.width=v+"px";k.style.height=w+"px"}var j=b.performance.now(),d=j-E;E=j;f.pausing||(a.exec(d/1E3),a.render());b.requestAnimationFrame(g);0===F%60&&(G=Math.round(6E4/(j-
x)),x=j,F=0);if(f.debug||e.debug)document.getElementById("lwf_info"+e.debugInfoElementId).innerHTML=G+"fps";F++;f.setCurrentFPS(G)}}catch(m){f.handleException_(m,e)}};d=function(c){try{if(C&&c.preventDefault(),a){var b,d,g=l.getBoundingClientRect();if(m){var i=c.touches[0];b=i.pageX;d=i.pageY}else b=c.clientX,d=c.clientY;m?(b-=l.offsetLeft,d-=l.offsetTop,b=b-g.left-window.scrollX,d=d-g.top-window.scrollY):(b-=g.left,d-=g.top);b/=u;d/=u;a.inputPoint(b,d)}}catch(h){f.handleException_(h,e)}};t=function(c){try{if(C&&
c.preventDefault(),a){var b,d,g=l.getBoundingClientRect();if(m){var i=c.touches[0];b=i.pageX;d=i.pageY}else b=c.clientX,d=c.clientY;m?(b-=l.offsetLeft,d-=l.offsetTop,b=b-g.left-window.scrollX,d=d-g.top-window.scrollY):(b-=g.left,d-=g.top);b/=u;d/=u;a.inputPoint(b,d);a.inputPress()}}catch(h){f.handleException_(h,e)}};h=function(c){try{C&&c.preventDefault(),a&&a.inputRelease()}catch(b){f.handleException_(b,e)}};D=function(c){try{a&&b.location.reload(!0)}catch(d){f.handleException_(d,e)}};b.requestAnimationFrame(g);
m?(p&&(y||/ SC-0/.test(k))&&document.body.addEventListener("touchstart",function(){}),A&&(f.debug||e.debug)&&c.addEventListener("gestureend",D,!1),c.addEventListener("touchmove",d,!1),c.addEventListener("touchstart",t,!1),c.addEventListener("touchend",h,!1)):(c.addEventListener("mousemove",d,!1),c.addEventListener("mousedown",t,!1),c.addEventListener("mouseup",h,!1));_.each(e.buttonEventMap,function(c,b){_.isFunction(c)?c={release:c}:_.each(c,function(a,c){if(!_.isFunction(a))throw Error("button event ["+
